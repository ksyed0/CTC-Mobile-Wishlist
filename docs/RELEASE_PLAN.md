...
Acceptance Criteria:
  - [x] AC-0045: On a physical device, scan screen behaves exactly as before (camera + live barcode scanning)
  - [x] AC-0046: On a simulator (Device.isDevice === false), the camera view is replaced with a scrollable WebView rendering data/product-catalog-print.html
  - [x] AC-0047: Each product card in the WebView is tappable; tapping fires handleBarcode() with the card's barcode string
  - [x] AC-0048: A simulator-mode banner is shown at the top of the scan screen
```
