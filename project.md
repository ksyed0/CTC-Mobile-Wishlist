...
_Updated session 2026-04-10 (Session 15) — iOS simulator debugging. Discovered `expo start --ios` (Expo Go) fails on this machine — iOS 18 simulator cannot connect to Metro via LAN IP or localhost. Root cause: native `ios/` build path was always used before; Expo Go networking is unreliable on this setup. Fix: always use `npx expo run:ios`. Bumped Expo packages to SDK 55 compatible versions (expo, expo-router, expo-splash-screen, expo-camera, expo-asset, expo-constants, expo-device, expo-file-system, expo-linking). BUG-116 logged. README updated: SDK 52 → 55, test count 246 → 459. Gotcha added to CLAUDE.md. All 459 tests pass._
...
- **iOS simulator launch:** always use `npx expo run:ios`, never `npx expo start --ios` — Expo Go fails to connect to Metro on this machine (iOS 18 simulator networking). The native `ios/` Xcode project must be used directly (see BUG-116)
