...
const fixture = fs.readFileSync(
  path.join(__dirname, "../fixtures/AI_COST_LOG.md"),
  "utf8",
);
...
describe("parseCostLog", () => {
  ...
  it("parses inputTokens as number", () =>
    expect(rows[0].inputTokens).toBe(45000));
  it("parses outputTokens as number", () =>
    expect(rows[0].outputTokens).toBe(12000));
  it("parses cacheReadTokens as number", () =>
    expect(rows[0].cacheReadTokens).toBe(8000));
  it("parses costUsd as number", () =>
    expect(rows[0].costUsd).toBeCloseTo(0.42));
});
