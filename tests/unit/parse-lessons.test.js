...
## L-001 — Jest upgrade eliminates transitive deprecation warnings
...
## L-007 — All config paths must use lowercase
...
## L-011 — HTML-escape all user-supplied strings before DOM injection
**Rule:** Every field sourced from user-controlled markdown files must be run through an HTML-escape helper.
*Learned when BUG-005 showed that unescaped script tag in a story title executed on page load.*
**Date:** 2026-03-10
`;
...
describe('parseLessons', () => {
  ...
  it('parses L-001 id correctly', () => expect(lessons[0].id).toBe('L-001'));
  it('parses L-007 id correctly', () => expect(lessons[1].id).toBe('L-007'));
  ...
  it('preserves document order (non-sequential IDs)', () => {
    expect(lessons.map((l) => l.id)).toEqual(['L-001', 'L-007', 'L-011']);
  });
