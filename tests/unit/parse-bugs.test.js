...
const fixture = fs.readFileSync(
  path.join(__dirname, "../fixtures/BUGS.md"),
  "utf8",
);
...
describe("parseBugs", () => {
  ...
  it("parses relatedStory", () =>
    expect(result[0].relatedStory).toBe("US-0001"));
  it("parses status", () => expect(result[0].status).toBe("Open"));
  it("parses fixBranch", () =>
    expect(result[0].fixBranch).toBe("bugfix/BUG-0001-empty-file-crash"));
  it("parses lessonEncoded", () => expect(result[0].lessonEncoded).toBe("No"));
  it("parses estimatedCostUsd", () =>
    expect(result[0].estimatedCostUsd).toBeCloseTo(0.35));
