...
const fixture = fs.readFileSync(
  path.join(__dirname, "../fixtures/progress.md"),
  "utf8",
);
...
describe("parseRecentActivity", () => {
  ...
  it("parses session date", () =>
    expect(result[0].date).toMatch(/\d{4}-\d{2}-\d{2}/));
  it("parses summary text", () =>
    expect(result[0].summary).toMatch(/FileSystemBridge/));
...
describe("parseRecentActivity — edge cases", () => {
  it("uses default limit of 5 when not specified", () => {
    const md = require("fs").readFileSync(
      require("path").join(__dirname, "../fixtures/progress.md"),
      "utf8",
    );
