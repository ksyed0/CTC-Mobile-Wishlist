...
describe("computeProjectedCost", () => {
  it("S = $400", () =>
    expect(computeProjectedCost("S", HOURS, RATE)).toBe(400));
  it("M = $800", () =>
    expect(computeProjectedCost("M", HOURS, RATE)).toBe(800));
  it("L = $1600", () =>
    expect(computeProjectedCost("L", HOURS, RATE)).toBe(1600));
  it("XL = $3200", () =>
    expect(computeProjectedCost("XL", HOURS, RATE)).toBe(3200));
  it("unknown size returns 0", () =>
    expect(computeProjectedCost("XXL", HOURS, RATE)).toBe(0));
  it("empty estimate returns 0", () =>
    expect(computeProjectedCost("", HOURS, RATE)).toBe(0));
});
...
describe("attributeBugCosts", () => {
  ...
  it("uses estimatedCostUsd fallback when branch has no cost log entry", () => {
    const bugsWithEstimate = [
      {
        id: "BUG-0003",
        fixBranch: "bugfix/BUG-0003-no-match",
  ...
  it("does not set isEstimated when estimatedCostUsd is 0", () => {
    const bugsNoEstimate = [
      { id: "BUG-0004", fixBranch: "bugfix/BUG-0004-no-match" },
    ];
