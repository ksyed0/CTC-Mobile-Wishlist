...
async function main() {
  ...
  if (!transcriptPath && sessionId) {
    try {
      ...
      if (
        ...
        fs.statSync(claudeProjects).isDirectory()
      ) {
        const projectDirs = fs.readdirSync(claudeProjects, {
          withFileTypes: true,
        });
  ...
  try {
    if (fs.fstatSync(fd).size === 0)
      fs.writeSync(fd, Buffer.from(HEADER, "utf8"));
...
main().catch((err) =>
  process.stderr.write(`[capture-cost] Error: ${err.message}\n`),
);
