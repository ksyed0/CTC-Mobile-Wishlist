  ...
  try {
    ...
    Object.keys(raw).forEach((k) => {
      if (!KNOWN_KEYS.includes(k))
        console.warn(`[generate-plan] Unknown config key: "${k}" — ignored`);
    });
    return {
    ...
    console.warn(
      "[generate-plan] Failed to parse plan-visualizer.config.json; using defaults.",
      e.message,
    );
    return DEFAULTS;
  }
...
function main() {
  ...
  const { epics, stories, tasks } = parseReleasePlan(
    readFile(config.docs.releasePlan),
  );
  ...
  const hasRealCosts = Object.values(aiAttribution).some(
    (c) => c && c.costUsd > 0,
  );
  ...
  for (const story of stories) {
    ...
    if (story.status === "Done" || story.status === "In Progress") {
      ...
      projectedUsd = estimateStoryCost(
        story.estimate,
        avgTokens,
        TOKEN_RATES.input,
        TOKEN_RATES.output,
    ...
    costs[story.id] = {
      ...
      inputTokens: aiAttribution[story.id]
        ? aiAttribution[story.id].inputTokens
        : 0,
      outputTokens: aiAttribution[story.id]
        ? aiAttribution[story.id].outputTokens
        : 0,
    };
  }
  ...
  for (const bug of bugs) {
    if (costs._bugs[bug.id]) {
      ...
      costs._bugs[bug.id].projectedUsd = computeProjectedCost(
        size,
        HOURS,
        RATE,
      );
  ...
  if (snapshots.length < 2) {
    console.log(
      "[generate-plan] Less than 2 snapshots found, attempting historical backfill...",
    );
