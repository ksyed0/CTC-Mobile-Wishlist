...
function computeBudgetMetrics(data, config, snapshots) {
  ...
  const explicitTotalBudget =
    budget.totalUsd !== null && budget.totalUsd !== undefined;
  ...
  const plannedProjected = (data.stories || [])
    ...
    .reduce(
      (sum, st) =>
        sum + ((data.costs[st.id] && data.costs[st.id].projectedUsd) || 0),
      0,
    );
  ...
  if (snapshots && snapshots.length >= 2) {
    ...
    if (recent.length >= 2) {
      ...
      const costDiff =
        (latest.data.costs._totals?.costUsd || 0) -
        (oldest.data.costs._totals?.costUsd || 0);
      const daysDiff = Math.max(
        1,
        Math.ceil(
          (new Date(latest.generatedAt) - new Date(oldest.generatedAt)) /
            (1000 * 60 * 60 * 24),
        ),
      );
  ...
  const crossedThresholds = thresholds.filter(
    (t) => percentUsed !== null && percentUsed >= t,
  );

  const epicBudgets = (data.epics || [])
    .map((epic) => {
      const epicStories = (data.stories || []).filter(
        (s) => s.epicId === epic.id,
      );
      const spent = epicStories.reduce(
        (sum, st) =>
          sum + ((data.costs[st.id] && data.costs[st.id].costUsd) || 0),
        0,
      );
      const epicPlanned = epicStories.filter(
...
function generateBudgetCSV(data, budgetMetrics, snapshots) {
  ...
  const burnRate = (
    budgetMetrics.burnRate !== null ? budgetMetrics.burnRate : 0
  ).toFixed(2);
  const exhaustion =
    budgetMetrics.daysRemaining !== null
  ...
  for (const epic of budgetMetrics.epicBudgets || []) {
    ...
    const remaining =
      epic.remaining !== null ? Number(epic.remaining).toFixed(2) : "";
    const pct = epic.percentUsed !== null ? `${epic.percentUsed}%` : "";

    lines.push(
      `${today},${epic.id},"${epic.title}",${budget},${spent},${remaining},${pct},${burnRate},${exhaustion}`,
    );
  }
  ...
  if (snapshots && snapshots.length > 0) {
    ...
    for (const snap of sorted) {
      ...
      for (const epic of data.epics || []) {
        const epicStories = (snap.data.stories || []).filter(
          (s) => s.epicId === epic.id,
        );
        const spent = epicStories.reduce(
          (sum, st) =>
            sum + ((snapCosts[st.id] && snapCosts[st.id].costUsd) || 0),
          0,
        );
        const epicBudget = epicMap[epic.id];
        const budget =
          epicBudget && epicBudget.budget !== null
            ? Number(epicBudget.budget).toFixed(2)
            : "";
        const remaining =
          epicBudget && epicBudget.budget !== null
            ? (Number(epicBudget.budget) - spent).toFixed(2)
            : "";
        const pct =
          epicBudget && epicBudget.percentUsed !== null
            ? `${epicBudget.percentUsed}%`
            : "";

        lines.push(
          `${date},${epic.id},"${epic.title}",${budget},${Number(spent).toFixed(2)},${remaining},${pct},,`,
        );
      }
    }
  }
